...
public class MenuRepository extends AbstractJavaRepository<Menu, Long> {
    ...
    /**
     * 加载可下发给前端路由的菜单(仅 M 目录/C 菜单、visible，不含按钮)。
     *
     * @param roleIds null 表示全部(超级管理员)；非空时取角色集合的菜单并集
     */
    public List<Menu> findRouteMenus(@Nullable Collection<Long> roleIds, @Nullable Fetcher<Menu> fetcher) {
        return sql
                .createQuery(table)
                .where(table.type().in(List.of("M", "C")))
                .where(table.visible().eq(true))
                .where(roleIds == null ? null : table.roles(role -> role.id().in(roleIds)))
                .orderBy(table.sortOrder().asc(), table.id().asc())
                .select(table.fetch(fetcher))
                .execute();
    }
